...
±±≥Programa  ≥ BOLITAU   ≥ Autor ≥                     ≥ Data ≥ 25/06/2015 ≥±±
...
User Function BOLITAU()
	...
	If Pergunte(cPerg,.T.)
		...
		*/
		...
		cFilter		+= "E1_PARCELA>='" + Alltrim(MV_PAR05) + "'.And.E1_PARCELA<='" + MV_PAR06 + "'.And."
...
User Function RFINR062(xBanco,xAgencia,xConta,xSubCt,lSetup,cNotIni,cNotFim,cSerie)
	...
	*/
	...
	Local aDadosEmp    := {"CORA RESIDENCIAL SENIOR"	,;//{	AllTrim(SM0->M0_NOMECOM)  	,;	//	[1]	Nome da Empresa
	"RUA IBIAPINOPOLIS,55"								,;	//	[2]	EndereÁo
	"JARDIM PAULISTANO, SAO PAULO, SP"	                ,;	//	[3]	Complemento
	"CEP: 01457-030"				                    ,;	//	[4]	CEP
	"PABX/FAX: "+SM0->M0_TEL							,;	//	[5]	Telefones  //nao est· sendo exibido  
	"CNPJ: 17.070.693/0001-91" 				            ,;	//	[6]
	"I.E.: "+Subs(SM0->M0_INSC,1,3)+"."+Subs(SM0->M0_INSC,4,3)+"."+			 ;	//	[7]
	Subs(SM0->M0_INSC,7,3)+"."+Subs(SM0->M0_INSC,10,3)							 }	//	[7]	I.E  //nao est· sendo exibido
	...
	"IntruÁıes de responsabilidade do " + Iif(AllTrim(GetNewPar("AB_CEDBEN", "C")) == "C
	", "Cedente", "Benefici·rio") + ". Qualquer d˙vida sobre este boleto contate o " + Iif(AllTrim(GetNewPar("AB_CEDBEN", "C")) == "C", "Cedente", "Benefici·rio") + "." ;
	} 


	Local aBolText := {;
	"*** VALORES EXPRESSOS EM REAIS ***", ;
	"ApÛs o vencimento, acrescer multa de 2% sobre o principal e mora de 1% ao mÍs ou fraÁ„o de mÍs. ", ;
	"CobranÁa escritural.",;
	"CrÈdito dado em garantia ao Banco Ita˙ S.A., pagar somente em banco.", ;
	...
	*/
	...
	Local aBolText := {;
	"*** VALORES EXPRESSOS EM REAIS ***", ;
	"ApÛs o vencimento, acrescer multa de 2% sobre o principal e mora de 1% ao mÍs ", ;
	"ou fraÁ„o de mÍs.",;
	"", ;
	"" ;
	}
	...
	While !EOF()
		...
		*/
		...
		If Marked("E1_OK")
			...
			cConta   := SE1->E1_CONTA
			cSubct   := MV_PAR20//iif(xSubCt	==nil,MV_PAR20,xSubCt)
			...
			*/
			aDadosBanco  := {SEE->EE_CODIGO  										    ,;	//	[1]	Numero do Banco
			SUBSTR(SA6->A6_NOME,1,14)									,;	//	[2]	Nome do Banco
			RTRIM(SA6->A6_AGENCIA)      								,;	//	[3]	AgÍncia
			RTRIM(SA6->A6_NUMCON)                                       ,;	//	[4]	Conta Corrente
			SA6->A6_DVCTA                                       	  	,;	//	[5]	DÌgito da conta corrente
			SUBSTR(SEE->EE_SUBCTA,1,3)									,;  //	[6]	Codigo da Carteira
			SUBSTR(SEE->EE_AGENCIA,5,1)                                 }   //	[7]	Digito da Agencia
			...
			cCartBras                           ,;
			SUBSTR(SEE->EE_CONTA,3,7)            ) //Numero da Carteira
...
Static Function Impress(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN)
	...
	Local aLogobr   := "logobra.bmp"
	//Local alogobrs  := "logobrs.bmp"
	Local alogoitau  := "logoitau.bmp"
	...
	//oPrint:Say  (nRow1+0200,100 ,aDadosEmp[1] + " - " + aDadosEmp[4],oFont10)				//Nome + CNPJ
	...
	ElseIF aDadosBanco[1] == "341"
		//cString := AllTrim(GetNewPar("AB_CARITAU", "112"))+"/"+(substr(aDadosTit[6],1,8)) + "-" + (substr(aDadosTit[6],9,1))
		cString := AllTrim(MV_PAR20)+"/"+(substr(aDadosTit[6],1,8)) + "-" + (substr(aDadosTit[6],9,1))
	ElseIF aDadosBanco[1] == "033"
	...
	Elseif aDadosBanco[1] == "341"
		...
		//oPrint:Say  (nRow2+1010,555 , AllTrim(GetNewPar("AB_CARITAU", "")),oFont10)
		oPrint:Say  (nRow2+1010,555 , AllTrim(MV_PAR20),oFont10)
	Elseif aDadosBanco[1] == "356"
		oPrint:Say  (nRow2+1010,555 ,"20",oFont10)
	Elseif aDadosBanco[1] == "033"
		oPrint:Say  (nRow2+1010,100 ,"101 - Rapido Com Registro",oFont10)
	Elseif aDadosBanco[1] == "237"
		oPrint:Say  (nRow2+1010,555 , AllTrim(GetNewPar("AB_CARTBRA", "09")) ,ofont10)
	Elseif aDadosBanco[1] == "001"
	...
	*/
	...
	oPrint:Say  (nRow2+1430,400 ,LEFT(aDatSacado[1],45),oFont10) //oPrint:Say  (nRow2+1430,400 ,LEFT(aDatSacado[1],45)+" ("+aDatSacado[2]+")",oFont10)
	...
	Elseif aDadosBanco[1] == "341"
		//oPrint:Say  (nRow3+2300,555, AllTrim(GetNewPar("AB_CARITAU", "")),oFont10)
		oPrint:Say  (nRow3+2300,555, AllTrim(MV_PAR20),oFont10)
	...
	Elseif aDadosBanco[1] == "237"
		oPrint:Say  (nRow3+2300,555, AllTrim(GetNewPar("AB_CARTBRA", "09")),oFont10)
	Elseif aDadosBanco[1] == "001"
	...
	*/
	...
	oPrint:Say  (nRow3+2700,400 ,LEFT(aDatSacado[1],45),oFont10)
	...
	If Empty(SE1->E1_NUMBCO)
		...
		Elseif SE1->E1_PORTADOR == "341"
			SE1->E1_NUMBCO 	:= Substr(aCB_RN_NN[3],1,9)//Substr(aCB_RN_NN[3],2,8)
		Else
...
Static Function Ret_cBarra(cBanco,cAgencia,cConta,cDacCC,cCarteira,cNroDoc,nValor,dvencimento,cConvenio,cSequencial,_lTemDesc,_cParcela,_cAgCompleta,cContra,cCartBras,_cCedente)
	...
	Local _cDigito := ""
	Local _cSuperDig := ""
	//Local _cCartIta  := AllTrim(GetNewPar("AB_CARITAU", "112"))//"109"      //numero do banco Ita˙
	Local _cCartIta  := AllTrim(MV_PAR20)//"109"      //numero do banco Ita˙
	Local _cCartBra  := AllTrim(GetNewPar("AB_CARTBRA", "09"))//"09"       //numero do banco Bradesco
	...
	Elseif Substr(cBanco,1,3) == "341"
		...
		//bldocnufinal := StrZero(Val(right(cNroDoc,8)+"0"), 8) //+ strzero(val(_cParcel),2)
...
Static Function ValidPerg()
	...
	cPerg   := PADR(cPerg,10)    // Passar para 10 no Protheus 10
	...
	aAdd(aRegs,{cPerg,"07","De Portador    ?","","","mv_ch7","C",3,0,0,"G","","MV_PAR07","","","",""        ,"","","","","","","","","","","","","","","","","","","","","SA6_0","","","",""})
	aAdd(aRegs,{cPerg,"08","Ate Portador   ?","","","mv_ch8","C",3,0,0,"G","","MV_PAR08","","","","ZZZ"     ,"","","","","","","","","","","","","","","","","","","","","SA6_0","","","",""})
	aAdd(aRegs,{cPerg,"09","De Cliente     ?","","","mv_ch9","C",6,0,0,"G","","MV_PAR09","","","",""        ,"","","","","","","","","","","","","","","","","","","","","SA1"  ,"","","",""})
	aAdd(aRegs,{cPerg,"10","De Loja        ?","","","mv_chA","C",2,0,0,"G","","MV_PAR10","","","",""        ,"","","","","","","","","","","","","","","","","","","","",""     ,"","","",""})
	aAdd(aRegs,{cPerg,"11","Ate Cliente    ?","","","mv_chB","C",6,0,0,"G","","MV_PAR11","","","","ZZZZZZ"  ,"","","","","","","","","","","","","","","","","","","","","SA1"  ,"","","",""})
	aAdd(aRegs,{cPerg,"12","Ate Loja       ?","","","mv_chc","C",2,0,0,"G","","MV_PAR12","","","","ZZ"      ,"","","","","","","","","","","","","","","","","","","","",""     ,"","","",""})
	...
	aAdd(aRegs,{cPerg,"19","Banco          ?","","","mv_chj","C",3,0,1,"G","","MV_PAR19","","","",""        ,"","","","","","","","","","","","","","","","","","","","","SA6_0","","","",""})
	aAdd(aRegs,{cPerg,"20","Carteira       ?","","","mv_chk","C",3,0,1,"G","","MV_PAR20","","","","109"     ,"","","","","","","","","","","","","","","","","","","","","","","","",""})
