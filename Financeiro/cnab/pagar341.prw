...
User Function Pagar341(_cOpcao)   
	...
	Do Case
		...
		If sa2->a2_banco == "341"
			If SEA->EA_MODELO =="02" .Or. SEA->EA_MODELO == "10"
				_cRetorno := IIF(SEA->EA_MODELO == "10","0"+strzero(val(substr(sea->ea_agedep,1,4)),4)+" "+"0000000","0"+strzero(val(substr(sa2->a2_agencia,1,4)),4)+" "+"0000000")
				_cRetorno += strzero(val(substr("0",6,5)),5)+" "+substr(Alltrim("0"),1,1)                               
			Else
				_cRetorno := "0"+strzero(val(substr(sa2->a2_agencia,1,4)),4)+" "+"0000000"
				_cRetorno += strzero(val(substr(_cConta,6,5)),5)+" "+substr(Alltrim(SA2->A2_DVCTA),1,1)                               
			EndIf
		Else
			If SEA->EA_MODELO =="02" .Or. SEA->EA_MODELO == "10"
				_cRetorno := strzero(val(substr(sa2->a2_agencia,1,5)),5)+" "
				_cRetorno += strzero(val(substr("0",1,12)),12)
				_cRetorno += " "+substr(Alltrim("0"),1,1)
			Else
				_cRetorno := strzero(val(substr(sa2->a2_agencia,1,5)),5)+" "
				_cRetorno += strzero(val(substr(_cConta,1,12)),12)
				If len(alltrim(SA2->A2_DVCTA)) > 1
					_cRetorno += strzero(val(substr(Alltrim(SA2->A2_DVCTA),1,2)),2)
				Else
					_cRetorno += " "+substr(Alltrim(SA2->A2_DVCTA),1,1)
				EndIf
			EndIf
		EndIf

		If SEA->EA_MODELO <> '10' .And. SEA->EA_TIPOPAG <> '20'
			//--- Mensagem ALERTA
		...
		Case _cTipo == "PP009"	//  Valor Pagamento liquido
		...
		Case _cTipo == "PP010"	//  Valor Pagamento liquido Imposto Municipal  -- Marcus Ferraz - 2016-09-09

		if SEA->EA_MODELO $ "13"
			_Liqui    := (SE2->E2_SALDO-SE2->E2_DECRESC+SE2->E2_ACRESC)
		else		 
			_Liqui    := SE2->(E2_SALDO+E2_XMULTA+E2_E_JUROS-E2_XDESCON)
		endif
		...
		If SEA->EA_MODELO == "16"    
			...
			//_cRetorno += Strzero( ( ( SE2->E2_SALDO - SE2->E2_DECRESC ) + SE2->E2_ACRESC ) * 100, 14 ) //Ajuste no campo decrescimo
			_cRetorno += Strzero((SE2->E2_SALDO-SE2->E2_DECRESC-SE2->E2_E_VLENT)*100,14) //Ajuste no campo decrescimo
			...
			_cRetorno += STRZERO(( ( SE2->E2_SALDO - SE2->E2_DECRESC )+ SE2->E2_ACRESC + SE2->E2_E_VLENT ) * 100,14)           
		...
		ElseIf (SEA->EA_MODELO == "17")   
			...
			_cRetorno += Strzero((SE2->E2_SALDO-SE2->E2_E_VLENT-SE2->E2_DECRESC)*100,14)
			//_cRetorno += Strzero((SE2->E2_SALDO-SE2->E2_DECRESC)*100,14)
			...
			_cRetorno += Strzero(( ( SE2->E2_SALDO - SE2->E2_DECRESC ) +SE2->E2_XMULTA+SE2->E2_E_JUROS)*100,14)
		...
		ElseIf SEA->EA_MODELO == "35" 
			...
			// Posicao 024 a 024: Tp Inscricao  2-CEI /  1-CNPJ               
			If !Empty(SE2->E2_XCNPJC)
...
/*/
User Function uSomaAbat(cPrefixo,cNumero,cParcela,cCart,nMoeda,dData,cFornCli,cLoja,cFilAbat,dDataRef,cTipo)
	#ifdef TOP
	Local cProcedure := IIF(__lGetSPName , GetSPName("FIN001","08"), "FIN001") //'SUMABAT'
	Local aResult 
	Local nTotAbat := 0
	Default cTipo:= ""
	...
	If lTitpaiSE1 .AND. cCart == "R"  
		If FindFunction("OrdTitpai") .and. (nOrdTitPai:= OrdTitpai()) > 0
			...
			If	DbSeek(cFilAbat+cPrefixo+cNumero+cParcela+cTipo+cFornCli+cLoja)
				bWhile := {|| !Eof() .And. Alltrim(cFilAbat+&cArea._TITPAI) ==  Alltrim(cFilAbat+cPrefixo+cNumero+cParcela+cTipo+cFornCli+cLoja) }
			Else
				DbSetOrder(1)
				DbSeek(cFilAbat+cPrefixo+cNumero+cParcela)
			Endif
